...
- (void)receiveDidStopWithStatus:(NSString *)statusString
{
    ...
    NSDictionary *dictionary = [NSDictionary dictionaryWithObjectsAndKeys:statusString, @"status", self.contentType, @"contentType", self.serverData, @"data", self.urlPath, @"urlpath",nil];
...
{    
    ...
    if([self.contentType compare:@"text/file"] == NSOrderedSame) {
        [httpClient postPathForFile:method parameters:self.params success:^(AFHTTPRequestOperation *operation, id responseObject)
         {
        ...
        [httpClient postPath:method parameters:self.params success:^(AFHTTPRequestOperation *operation, id responseObject)
         {
             //NSString *responseStr = [[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding];
             //NSLog(@"Request Successful, response '%@'", responseStr);
