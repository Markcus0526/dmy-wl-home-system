...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSMutableArray *)imgArr TitleArray:(NSArray *)titArr
{
    
	if ((self=[super initWithFrame:rect])) {
        self.userInteractionEnabled=YES;
        titleArray=[titArr retain];
        NSMutableArray *tempArray=imgArr;
        ...
        for (int i=0; i<pageCount; i++) {
            ...
            if ([imgURL hasPrefix:@"http://"]) {
                ...
                [imgView setImageWithURL:[NSURL URLWithUnicodeString:imgURL]];
            }
            else
            {
...
//    pageControl.currentPage=(page-1);
	if ([delegate respondsToSelector:@selector(EScrollerViewDidChanged:)]) {
        [delegate EScrollerViewDidChanged:(page-1)];
    }
