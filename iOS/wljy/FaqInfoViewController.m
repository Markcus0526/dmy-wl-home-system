...
#define kBottomBottomBackY         113

@implementation FaqInfoViewController
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if([self.pageClass compare:@"faq"] == NSOrderedSame) {
        ...
        [self.allAnswerButton setTitle:@"... 阅读全部回答" forState:UIControlStateNormal];
        self.recentAnswerLabel.text = @"       最佳回答";
        self.integralTitleLabel.hidden = NO;
        self.integralLabel.hidden = NO;
    } else if ([self.pageClass compare:@"study"] == NSOrderedSame) {
        self.viewTitleLabel.text = @"学习";
        [self.allAnswerButton setTitle:@"... 阅读全部回答" forState:UIControlStateNormal];
        self.recentAnswerLabel.text = @"       最佳回答";
        self.integralTitleLabel.hidden = YES;
        self.integralLabel.hidden = YES;
    } else {
        self.viewTitleLabel.text = @"分享";
        [self.allAnswerButton setTitle:@"... 阅读全部评论" forState:UIControlStateNormal];
        self.recentAnswerLabel.text = @"       最佳评论";
        self.integralTitleLabel.hidden = YES;
        self.integralLabel.hidden = YES;
    }
...
- (IBAction)onAllAnswer:(id)sender {
    ...
    viewController.pageClass = self.pageClass;
...
- (void)connectToServer {
    if(self.loadingIndicator.isAnimating == NO && self.loadingIndicator.hidden == YES) {
        ...
        [getController setUrlPath:[NSString stringWithFormat:@"%@_info.jsp", self.pageClass]];
    ...
    if([status compare:@"succeeded"] == NSOrderedSame) {
        if([contentType compare:@"text"] == NSOrderedSame) {
            ...
            if([success intValue] == 1) {
                NSArray *keys = @[@"title", @"provider_name", @"rewardintegral", @"body", @"postdate", @"answer_count", @"type", @"read_count", @"good_cnt", @"bad_cnt", @"answerer_name", @"answer_date", @"answer_body" , @"imagepath"];
                
                imagepathes = [NSMutableDictionary dictionary];
                
                NSMutableDictionary *faqInfo = [NSMutableDictionary dictionary];
                for(NSString *key in keys) {
                    id val = [jsonDic objectForKey:key];
                    if(val != nil && ![val isKindOfClass:[NSNull class]]) {
                        [faqInfo setObject:val forKey:key];
                ...
                self.titleLabel.text = [faqInfo objectForKey:[keys objectAtIndex:0]];
                self.providerNameLabel.text = [faqInfo objectForKey:[keys objectAtIndex:1]];
                self.integralLabel.text = [NSString stringWithFormat:@"%@", [faqInfo objectForKey:[keys objectAtIndex:2]]];
                
                self.bodyLabel.text = [faqInfo objectForKey:[keys objectAtIndex:3]];
                ...
                self.postdateLabel.text = [faqInfo objectForKey:[keys objectAtIndex:4]];
                
                if([self.pageClass compare:@"faq"] == NSOrderedSame) {
                    self.answerCountLabel.text = [NSString stringWithFormat:@"回答:%@", [faqInfo objectForKey:[keys objectAtIndex:5]]];
                } else if ([self.pageClass compare:@"study"] == NSOrderedSame) {
                    self.answerCountLabel.text = [NSString stringWithFormat:@"回答:%@", [faqInfo objectForKey:[keys objectAtIndex:5]]];
                } else {
                    self.answerCountLabel.text = [NSString stringWithFormat:@"评论:%@", [faqInfo objectForKey:[keys objectAtIndex:5]]];
                }
                
                
                self.typeLabel.text = [NSString stringWithFormat:@"所术分类:%@",[faqInfo objectForKey:[keys objectAtIndex:6]]];
                self.readCountLabel.text = [NSString stringWithFormat:@"☺ %@", [faqInfo objectForKey:[keys objectAtIndex:7]]];
                
                [self.evalUpButton setTitle:[NSString stringWithFormat:@"%@", [faqInfo objectForKey:[keys objectAtIndex:8]]] forState:UIControlStateNormal];
                [self.evalDownButton setTitle:[NSString stringWithFormat:@"%@", [faqInfo objectForKey:[keys objectAtIndex:9]]] forState:UIControlStateNormal];
                self.answerEvalLabel.text = [NSString stringWithFormat:@"%@赞 %@踩", [faqInfo objectForKey:[keys objectAtIndex:8]], [faqInfo objectForKey:[keys objectAtIndex:9]]];
                
                
                
                if([[faqInfo objectForKey:[keys objectAtIndex:5]] integerValue] > 0) {
                    self.answererNameLabel.text = [faqInfo objectForKey:[keys objectAtIndex:10]];
                    self.answerDateLabel.text = [NSString stringWithFormat:@"%@天前", [faqInfo objectForKey:[keys objectAtIndex:11]]];
                    
                    self.answerBodyLabel.text = [faqInfo objectForKey:[keys objectAtIndex:12]];
                    ...
                    [self.answererImageView setImageWithURL:[NSURL URLWithUnicodeString:[NSString stringWithFormat:@"%@%@", Server_Address, [faqInfo objectForKey:[keys objectAtIndex:13]]]] placeholderImage:[UIImage imageNamed:@"noimage.png"]];
...
- (void)postToServer:(NSInteger)evaltype {
    if(self.loadingIndicator.isAnimating == NO && self.loadingIndicator.hidden == YES) {
        ...
        [postController setUrlPath:[NSString stringWithFormat:@"%@_info_addcomment.jsp", self.pageClass]];
