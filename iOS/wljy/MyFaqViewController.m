...
- (void)connectToServer {
    if(self.loadingIndicator.isAnimating == NO && self.loadingIndicator.hidden == YES) {
        ...
        [getController setUrlPath:@"myquestion.jsp"];
...
- (void)connectionComplete:(NSNotification*)aNotification {
    ...
    if([status compare:@"succeeded"] == NSOrderedSame) {
        if([contentType compare:@"text"] == NSOrderedSame) {
            ...
            NSLog(@"MyFaq: %@, %@", status, [NSString stringWithFormat:@"%s", [data bytes]]);
            ...
            if([success intValue] == 1 && [count intValue] > 0) {
                NSArray *subkeys = @[@"id", @"title", @"body", @"postdate", @"read_count", @"answer_count", @"answerer_name"];
...
{
    static NSString *cellIdentifier = @"MyFaqCellIdentifier";
    
    MyFaqCell *cell = (MyFaqCell*)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    
    NSDictionary *rowData = [myFaqs objectAtIndex:[indexPath section]];
    [[cell titleLabel] setText:[rowData objectForKey:@"title"]];
    [[cell dateLabel] setText:[NSString stringWithFormat:@"%@", [rowData objectForKey:@"postdate"]]];
    ...
    //[[cell providerNameLabel] setText:[NSString stringWithFormat:@"提问: %d", [[rowData objectForKey:@"provider_name"] intValue]]];
    [[cell answererNameLabel] setText:[NSString stringWithFormat:@"最新回答: %@", [rowData objectForKey:@"answerer_name"]]];
