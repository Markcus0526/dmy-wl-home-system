...
- (IBAction)onPost:(id)sender {
    if(self.titleText.text.length < 1) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"分享错误" message:@"无标题!" delegate:nil cancelButtonTitle:@"确认" otherButtonTitles:nil];
        [alert show];
        return;
    }
    /*if(self.bodyText.text.length < 1) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"分享错误" message:@"无献策内容!" delegate:nil cancelButtonTitle:@"确认" otherButtonTitles:nil];
        [alert show];
        return;
    }*/
    /*if(self.filename == nil) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"分享错误" message:@"无附件资料!" delegate:nil cancelButtonTitle:@"确认" otherButtonTitles:nil];
        [alert show];
        return;
    }
...
- (void)postToServer {
    if(self.loadingIndicator.isAnimating == NO && self.loadingIndicator.hidden == YES) {
        ...
        [postController setUrlPath:@"share_add.jsp"];
        [postController setContentType:@"text/file"];
        NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d", appDelegate.userid], @"userid", self.titleText.text, @"share_title", [NSString stringWithFormat:@"%d", self.type], @"share_type", self.bodyText.text, @"share_body", self.filename, @"fileContent", nil];
...
- (void)connectionComplete:(NSNotification*)aNotification {
    ...
    if([status compare:@"succeeded"] == NSOrderedSame) {
        ...
        if ([success intValue] == 1) {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"分享失败" message:@"可能是服务机错误。" delegate:nil cancelButtonTitle:@"确认" otherButtonTitles:nil];
            [alert show];
        }
    } else {
...
- (void) callSubmitActivity
{
    ...
    if(self.loadingIndicator.isAnimating == NO && self.loadingIndicator.hidden == YES) {
        ...
        NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:
                                ...
                                self.titleText.text, @"share_title",
                                [NSString stringWithFormat:@"%d", self.type], @"share_type",
                                self.bodyText.text, @"share_body",
                                nil];
        
        NSString *method = Server_Address;
        method = [method stringByAppendingString:@"phone/"];
        method = [method stringByAppendingString:@"share_addmany.jsp"];
        ...
        AFHTTPRequestOperation *operation = [httpClient
                                             ...
                                             success:^(AFHTTPRequestOperation *operation, id responseObject)
                                             {
                                                 ...
                                                 if ([success intValue] == 1) {
                                                     ...
                                                     UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"发布分享失败" message:@"可能是服务机错误。" delegate:nil cancelButtonTitle:@"确认" otherButtonTitles:nil];
                                                     [alert show];
                                                 }
                                             }
