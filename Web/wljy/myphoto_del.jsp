...
<%
    ...
    if (id != 0) {
        sql = "update MEMBER_DATA set deleted = 1 where id = " + id;		
        stmt.executeQuery(sql);
		sql = "update MEMBER_IMAGE set deleted = 1 where rid = " + id;
		stmt.executeQuery(sql);
	}
    else
    {
        ...
        for ( i = 0; i < 8; i++ )
        {
            ...
            if ( ischeck == 1 )
            {
                sql = "update MEMBER_DATA set deleted = 1 where id = " + tmpId;
                stmt.executeQuery(sql);
				sql = "update MEMBER_IMAGE set deleted = 1 where rid = " + tmpId;
				stmt.executeQuery(sql);
            }
        }
    }
