...
<%
    ...
    if ( add_condition.equals(""))
    sql = "select count(*) as count from PROBLEM_QINFO where deleted = 0";
    else
        sql = "select count(*) as count from PROBLEM_QINFO where deleted = 0 and " + add_condition;
 ...
 int last_index = total_count -  (pagenum - 1) * pagecount;
    ...
    if ( add_condition.equals(""))
    sql = "select * from (select * from (select * from PROBLEM_QINFO where deleted = 0 order by id ASC)  where ID >= (select MAX(ID) from (select * from (select * from PROBLEM_QINFO where deleted = 0 order by id ASC) where ROWNUM <= " + last_index + ") where rownum <= " + first_index + ") and ROWNUM <= " + count + ") order by id DESC"; 
    else
        sql = "select * from (select * from (select * from PROBLEM_QINFO where deleted = 0 and " + add_condition + " order by id ASC)  where ID >= (select MAX(ID) from (select * from (select * from PROBLEM_QINFO where deleted = 0 and " + add_condition + " order by id ASC) where ROWNUM <= " + last_index + ") where rownum <= " + first_index + ") and ROWNUM <= " + count + ") order by id DESC"; 
    ...
    int rewardintegral, readcount, isVisible, is_public = 0;
...
function add_data() {    
    ...
    $("#increase_rewardintegral").attr("value", "0");    
...
function delete_data(id) {
    if (confirm("您要删除选择的问答?")) {
        $("#form_edit_action").attr("value", "delete");
        $("#form_edit_id").attr("value", id);
        $("#form_edit").submit();
    }
}
function modify_data(id, mid, title, type, body, postdate, rewardintegral, readcount, isVisible, ispublic) {
    ...
    $("#increase_rewardintegral").attr("value", rewardintegral); 
    
    $(".increase").show();
}
function submit_data() {
    if($("#increase_title").val().length > 0) {
        if($("#increase_rewardintegral").val().length > 0 && !isNaN($("#increase_rewardintegral").val()) && $("#increase_rewardintegral").val() <= parseInt('<%=max_integral%>' ,10)) {
        $("#form_edit_title").attr("value", $("#increase_title").val());
        $("#form_edit_type").attr("value", $("#increase_page").val());   
        $("#form_edit_rewardintegral").attr("value", $("#increase_rewardintegral").val());    
...
<body>
    <div class="header">
      <div class="includeDom" include="top.jsp"></div>
        <div class="nav-list">
            <ul>
                <li><a href="javascript:void(0);" class="on"><span>问答管理</span></a></li>
            </ul>
        </div>
        <div class="clear"></div>
    </div>
		...
		<div class="content-box">
                    <div class="content-title">
                        ...
                        <table width="100%">
                            <tr>
                                <td>	
			<h2>问答管理<a href="javascript:void(0)" onclick="add_data()">添加</a><a id="del" href="#1" onclick="select_delete();">删除</a></h2>
                                </td>
                                <td>
                                    <div style="align:center" align="center">
                                        <form action="faq.jsp" method="post" id="search_form">
...
<td class="checkboxes" ><input type="checkbox" value="<%=id%>"/></td>
				...
				<td><a href="#" id="public_<%=id%>" onclick="change_public('<%=id%>');"><%=public_string%></a></td>
                                ...
                                <td>
                                    <a href="javascript:void(0)" onclick="modify_data('<%=id%>', '<%=mid%>', '<%=title%>', '<%=type%>', '<%=body%>', '<%=postdate%>', '<%=rewardintegral%>', '<%=readcount%>', '<%=isVisible%>', '<%=is_public%>')">编辑</a> | 
            ...
            <table width="100%">
                ...
                <tr>
                    ...
                    <td class="drwrap">
                            ...
                            <div class="imgtablenone">
                                <table width="100%">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <%
                                            sql = "select * from PROBLEM_TYPE order by id";
                                    ...
                                    <tr>
                                        <td>悬赏积分 :</td>
                                        <td><input type="text" id="increase_rewardintegral" value="0" style="width: 80px;"/>&nbsp; 最大值为&nbsp;<%=max_integral%></td>
                                    </tr>
                                    <tr>
                                        <td>显隐:</td>
                                        <td>
                        ...
                        <div class="imgtext">
                            <h2>问答详情</h2>
                            <form action="faq_addinfo.jsp" method="post" id="form_edit">
                                ...
                                <input type="hidden" id="form_edit_type" name="type" value="1"/>
                                <input type="hidden" name="ids" id="ids" value=""/>
                                <input type="hidden" id="form_edit_ispublic" name="ispublic" value="1"/>
                                
                                <input type="hidden" id="form_edit_body" name ="body" value="" />
                                <input type="hidden" id="form_edit_readcount" name="readcount" value="0" />
                                <input type="hidden" id="form_edit_rewardintegral" name="rewardintegral" value="0" />
