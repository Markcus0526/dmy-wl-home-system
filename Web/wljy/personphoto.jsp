<%-- 
    Document   : personphoto
    Created on : 2013/12/15, 下午 01:41:58
    Author     : KIMSUI
--%>
...
<%
 Cookie[] cookies = request.getCookies();
  ...
  String username1="";
    ...
    String sql = "select count(*) as count from member_data where type = 0 and deleted = 0 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR') and mid = " + user_id;
    ...
    sql = "select * from (select * from (select * from member_data where type = 0 and deleted = 0 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR') and mid = '" + user_id + "' order by id) where ID >= (select MAX(ID) from (select * from (select * from member_data where type = 0 and deleted = 0 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR') and mid = '" + user_id + "' order by id) where ROWNUM <= " + last_index + ") where rownum <= " + first_index + ") and ROWNUM <= " + pagecount + ") order by ID DESC";
...
$(function() {
 ...
 function change_page(index) {
        window.location.href = "personphoto.jsp?id=<%=sid%>&page=" + index;            
}
			...
			<div class="exchangelist myphotolist marT20">
				<ul class="clear">
                                    ...
                                    while (rs.next()) {
                                        ...
                                        image_path = rs.getString("datapath");
                                %>
					<li align="center">
...
<script type="text/javascript">
$(function() {
	$(".postphotomore a").click(function() {
		i = parseInt($(".num:last").jsp()) +1;
		$(".myinfoupdate table").append('<tr><td width="110" align="right">照片<span class="num">'+ i +'</span>：</td><td><div class="clear"><input type="text" class="update_input fle"><input type="button" value="" class="update_btn fle" /><span class="fle">默认第一张为缩略图</span></div></td></tr><tr><td width="110" align="right" valign="top">描述：</td><td colspan="2"><textarea class="upadate_area"></textarea></td></tr>');
	})
})
</script>
</body>
