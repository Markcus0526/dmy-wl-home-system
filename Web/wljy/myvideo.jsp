<%-- 
    Document   : myvideo
    Created on : 10-Dec-2013, 10:13:29
    Author     : KIMSUI
--%>
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script type="text/javascript">
            ...
            function change_page(index) {
                window.location.href = "myvideo.jsp?page=" + index;            
            }
    ...
    <body>
        ...
        <%
            ...
            String sql = "select count(*) as count from share_info where deleted = 0 and mid = " + UserId;
            ...
            if (rs.next()) {
             ...
             /*   if (total_count == 0) {
                    out.println("<script>window.location.href='postvideo.jsp';</script>");
                    return;
                }*/
            ...
            sql = "select * from (select * from (select * from share_info where deleted = 0 and ISVISIBLE=1 and mid = '" + UserId + "' order by id) where ID >= (select MAX(ID) from (select * from (select * from share_info where deleted = 0 and ISVISIBLE=1 and mid = '" + UserId + "' order by id) where ROWNUM <= " + last_index + ") where rownum <= " + first_index + ") and ROWNUM <= " + pagecount + ") order by ID DESC";
        ...
        <div class="w964">
            ...
            <div class="site clear">
                ...
                <div class="bigmid">
                    <div class="singlebtn" align="right"><a href="sharemy.jsp">我 要 分 享</a></div>
                    <div class="photoEdit">
                        ...
                        <div class="myexchangelist photEditCon">
                            <ul>
                                <form action="myvideo_del.jsp" method="post" id="submit_form">
                                    <%
                                        int tmpInd = 0;
                                        while (rs.next()) {
											...
											String strQuery = "Select * from share_data where rid = " + videoid + " and deleted = 0";
											Statement queryStmt = conn.createStatement();
											ResultSet queryRS = queryStmt.executeQuery(strQuery);
											if (queryRS.next())
											{
												sdtype = queryRS.getInt("type");
												if (sdtype == 0) 
												{
                                    ...
                                    <li class="clear">
                                        <span style="width:20px;" class="editCheck">
                                            ...
                                            <input type="hidden" name="id_<%=tmpInd%>" value="<%=videoid%>" />
                                        </span>
                                        <span style="width:120px;"><img src="<%=image_path%>" width="102" height="77" alt=""></span>
                                        <span class="editTit" style="width:220px; text-align:center;"><%=title%></span>
                                        <span style="width:215px; text-align:center;"><%=postdate%></span>
                                        <span style="width:195px; text-align:center;"><a href="share_info.jsp?id=<%=videoid%>">查看详细</a> | <a href="javascript:;" class="editDelete" onclick="delete_act('<%=videoid%>')">删除</a> | <a href="sharemy_edit.jsp?id=<%=videoid%>">编辑</a></span>
                                    </li>
                                    <%
                                            tmpInd++;
                                        };
        ...
        <script>
            ...
            {
                if (confirm("您要删除选择的分享吗?"))
                {
                    window.location.href = "myvideo_del.jsp?id="+id;
                }
            }
