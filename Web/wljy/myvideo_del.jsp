...
<%
    ...
    if (id != 0) {
        sql = "update share_info set deleted = 1 where id = " + id;        
		...
		while (myrs.next())
		{
			...
			sql = "update share_data set deleted = 1 where rid = " + rid;        
	        stmt.executeQuery(sql);
		}
		myrs.close();
    }
    else
    {
        ...
        for ( i = 0; i < 8; i++ )
        {
            ...
            if ( ischeck == 1 )
            {
                sql = "update share_info set deleted = 1 where id = " + tmpId;
                stmt.executeQuery(sql);

				sql = "select * from share_data where rid = " + tmpId;
				ResultSet myrs = stmt.executeQuery(sql);
				while (myrs.next())
				{
					int rid = myrs.getInt("rid");
