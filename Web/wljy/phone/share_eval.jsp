...
<%
    ...
    if (!rs.next()) {
        ...
        sql = "select count(*) as COUNT from SHARE_EVAL where RID = " + id + " and ISVISIBLE=1 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR')";
        ...
        sql = "select * from (select * from (select * from SHARE_EVAL where RID = " + id + " and ISVISIBLE=1 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR') order by ID) where ID >= (select MAX(ID) from (select * from (select * from SHARE_EVAL where RID = " + id + " and ISVISIBLE=1 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR') order by ID) where ROWNUM <= " + last_index + ") where ROWNUM <= " + first_index + ") and ROWNUM <= " + pagecount + ") order by ID DESC";
        rs = stmt.executeQuery(sql);
        ...
        while (rs.next()) {
            ...
            else  body = clob.getSubString(1, clob_length);
