<%-- 
    Document   : faq_detail
    Created on : 2014/2/3, 下午 02:40:25
    Author     : kimsui
--%>
    ...
    if (!rs.next()) {
        ...
        if (rs.next()) {
            ...
            if (total_count != 0) {
                ...
                sql = "select * from (select * from  (select * from PROBLEM_AINFO where deleted = 0 and pid = '" + id + "' order by id ASC) where ID >= (select MAX(ID) from (select * from PROBLEM_AINFO where deleted = 0 and pid = '" + id + "' and ROWNUM <= " + last_index + " order by id) where rownum <= " + first_index + ") and ROWNUM <= " + pagecount + ") order by ID DESC";
                rs = stmt.executeQuery(sql);
                mid = 0;
