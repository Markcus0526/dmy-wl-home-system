...
<%
    ...
    if (!rs.next()) {
        ...
        sql = "select count(*) as cnt from activity_comment where mid = " + userid + " and aid = " + id;
        ...
        if (rs.next()) {
            if (rs.getInt("cnt") > 0) {
                ...
                sql = "select max(id) from activity_comment";
                rs = stmt.executeQuery(sql);
                int max_id = 1;
                if (rs.next()) {
                    max_id = rs.getInt(1);
                ...
                sql = "Insert into ACTIVITY_COMMENT (ID, AID, MID, TYPE) VALUES (?, ?, ?, ?)";
                ...
                if (Integer.parseInt(type) == 1) {
                    sql = "UPDATE ACTIVITY_INFO SET GOOD=GOOD+1 where ID=" + id;
                } else {
                    sql = "UPDATE ACTIVITY_INFO SET BAD=BAD+1 where ID=" + id;
                }
