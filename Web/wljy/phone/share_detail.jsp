    ...
    if (!rs.next()) {
        ...
        if (rs.next()) {
            ...
            if (total_count != 0) {
                ...
                sql = "select * from (select * from  (select * from SHARE_EVAL where rid = '" + id + "' order by id ASC) where ID >= (select MAX(ID) from (select * from SHARE_EVAL where rid = '" + id + "' and ROWNUM <= " + last_index + " order by id) where rownum <= " + first_index + ") and ROWNUM <= " + pagecount + ") order by ID DESC";
                rs = stmt.executeQuery(sql);
                mid = 0;
