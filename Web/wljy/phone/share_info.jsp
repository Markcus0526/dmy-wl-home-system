<%-- 
    Document   : study_info
    Created on : 2014/2/3, 下午 03:13:13
    Author     : kimsui
--%>
...
<%
    ...
    if (!rs.next()) {
        ...
        sql = "update SHARE_INFO set readcount = readcount + 1 where ID = " + id;
        stmt.executeUpdate(sql);

        sql = "select * from SHARE_INFO where ID = " + id;
        ...
        while (rs.next()) {
            ...
            sql = "select * from SHARE_TYPE where ID = " + itype;
            rs1 = u_stmt.executeQuery(sql);
            while (rs1.next()) {
        ...
        sql = "select * from SHARE_EVAL where RID = " + id + " order by ID DESC";
        rs1 = u_stmt.executeQuery(sql);
        if (rs1.next()) {
            mid = rs1.getInt("MID");
