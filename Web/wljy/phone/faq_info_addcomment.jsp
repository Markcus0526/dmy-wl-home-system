...
<%
    ...
    if (!rs.next()) {
        ...
        sql = "select count(*) as cnt from problem_comment where mid = " + userid + " and pid = " + id;
        ...
        if (rs.next()) {
            if (rs.getInt("cnt") > 0) {
                ...
                sql = "select max(id) from problem_comment";
                rs = stmt.executeQuery(sql);
                int max_id = 1;
                if (rs.next()) {
                    max_id = rs.getInt(1);
                ...
                sql = "Insert into problem_comment (ID, PID, MID, TYPE) VALUES (?, ?, ?, ?)";
                ...
                if (Integer.parseInt(type) == 1) {
                    sql = "UPDATE PROBLEM_QINFO SET GOOD=GOOD+1 where ID=" + id;
                } else {
                    sql = "UPDATE PROBLEM_QINFO SET BAD=BAD+1 where ID=" + id;
                }
