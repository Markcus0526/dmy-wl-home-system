<%-- 
    Document   : share
    Created on : 2014/1/16, 下午 04:56:18
    Author     : kimsui
--%>
...
<%
    ...
    if (!rs.next()) {
        ...
        int type = Integer.parseInt(request.getParameter("type") == null ? "1" : request.getParameter("type"), 10);
        ...
        sql = "select count(*) as count from SHARE_INFO where deleted = 0 and type = " + type + " and ISVISIBLE=1 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR')";
        ...
        sql = "select * from (select * from (select * from SHARE_INFO where deleted = 0 and type = " + type + " and ISVISIBLE=1 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR') order by id) where ID >= (select MAX(ID) from (select * from (select * from SHARE_INFO where deleted = 0 and type = " + type + " and ISVISIBLE=1 and TO_CHAR(postdate, 'YEAR') = TO_CHAR(sysdate, 'YEAR') order by id) where ROWNUM <= " + last_index + ") where rownum <= " + first_index + ") and ROWNUM <= " + pagecount + ") order by ID DESC";
        rs = stmt.executeQuery(sql);
        ...
        while (rs.next()) {
            ...
            sql = "Select * from share_data where rid = " + id;
            rs1 = u_stmt.executeQuery(sql);
            if (rs1.next()) {
            ...
            sql = "select count(*) as count from share_eval where rid = " + id;
            rs1 = u_stmt.executeQuery(sql);
            if (rs1.next()) {
            ...
            obj1[index].put("readcount", readcount);
            obj1[index].put("postdate", postdate.toString());
            obj1[index].put("username", act_user);
