...
<%
    ...
    if (!rs.next()) {
        ...
        sql = "update STUDY_QINFO set readcount = readcount + 1 where ID = " + id;
        ...
        while (rs.next()) {
            ...
            sql = "select * from STUDY_TYPE where ID = " + itype;
            rs1 = u_stmt.executeQuery(sql);
            while (rs1.next()) {
            ...
            if (clob_len < 0) body = "";
            else  body = clob.getSubString(1, clob_len);
        ...
        sql = "select * from STUDY_AINFO where SID = " + id + " order by ID DESC";
        rs1 = u_stmt.executeQuery(sql);
        if (rs1.next()) {
            mid = rs1.getInt("MID");
            ...
            clob_len = (clob==null)?-1:(int)clob.length();
            if (clob_len < 0)  body = "";
            else answer_body = clob.getSubString(1, clob_len);
