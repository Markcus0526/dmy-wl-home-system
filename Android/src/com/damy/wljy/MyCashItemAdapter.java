...
class MyCashItemAdapter extends BaseAdapter {
	Context			mContext = null;
	MyCashActivity 	mActivity = null;
    private LayoutInflater mInflater;
    private Bitmap bitmapNoImage;

    public MyCashItemAdapter(MyCashActivity activity, Context context) {
    	mContext = context;
    	mActivity = activity;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
    	...
    	MyCashItemHolder anHolder;

        // When convertView is not null, we can reuse it directly, there is no need
        // to reinflate it. We only inflate a new View when the convertView supplied
        // by ListView is null.
        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.activity_mycash_item, null);

            // Creates a ViewHolder and store references to the two children views
            // we want to bind data to.
            anHolder = new MyCashItemHolder();
            
            anHolder.lab_title = (AutoSizeTextView)convertView.findViewById(R.id.lab_mycash_item_title);
            anHolder.lab_marketprice = (AutoSizeTextView)convertView.findViewById(R.id.lab_mycash_item_cost);
            anHolder.lab_integralprice = (AutoSizeTextView)convertView.findViewById(R.id.lab_mycash_item_integral);
            anHolder.lab_state = (AutoSizeTextView)convertView.findViewById(R.id.lab_mycash_item_state);
            anHolder.img_item_image = (ImageView)convertView.findViewById(R.id.img_mycash_item_img);

            convertView.setTag(anHolder);
        } else {
            // Get the ViewHolder back to get fast access to the TextView
            // and the ImageView.
        	anHolder = (MyCashItemHolder) convertView.getTag();
        }

        STMyCashItemInfo anItem = mActivity.getItem(position);
        ...
        if (anItem != null)
        {
            ...
            anHolder.lab_state.setText(anItem.state);
